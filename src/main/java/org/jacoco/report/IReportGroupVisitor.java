...
/**
 * Output-Interface for hierarchical org.jacoco.report structures. To allow sequential
 * processing and save memory the group structure has to be traversed in a
 * "deep first" fashion. The interface is implemented by the org.jacoco.report formatters
 * and can be used to emit coverage org.jacoco.report structures.
 * 
...
public interface IReportGroupVisitor {
	...
	/**
	 * Called to add a bundle to the the org.jacoco.report.
	 * 
	 * @param bundle
	 *            a bundle to include in the org.jacoco.report
	 * @param locator
	 *            source locator for this bundle
	 * @throws IOException
	 *             in case of IO problems with the org.jacoco.report writer
	 */
	void visitBundle(IBundleCoverage bundle, ISourceFileLocator locator)
			throws IOException;

	/**
	 * Called to add a new group to the org.jacoco.report. The returned
